...
String payloadPrintBuffer[BUFFER_SIZE]; // Buffer khusus payload untuk printer
   ...
   case TRANSMITTING:
    ...
    if (Serial1.available()) {
        ...
        if (!serialData.isEmpty()) {
            ...
            // Simpan data penuh ke buffer pengiriman
            if (bufferIndex < BUFFER_SIZE) {
                payloadBuffer[bufferIndex++] = serialData; // Data penuh
            } else {
            ...
            // Pisahkan payload dari serialData
            String parts[6];
            int index = 0;
            ...
            // Simpan hanya payload ke buffer pencetakan
            if (!parts[4].isEmpty() && bufferIndex <= BUFFER_SIZE) {
                payloadPrintBuffer[bufferIndex - 1] = parts[4];
            }
        }
    }
...
void printLast10Data() {
    ...
    for (int i = 0; i < BUFFER_SIZE; i++) {
        if (!payloadPrintBuffer[i].isEmpty()) {
            // Cetak hanya payload
            printer.printf("09:00    1       %s\n", payloadPrintBuffer[i].c_str());
        }
    }
